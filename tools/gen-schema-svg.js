/**
 * 生成 Victor's CRM 数据库 Schema 关系图 (SVG) —— 唯一事实来源
 * 数据来源: 迁移文件 + 生产库实测；每次 schema 变更后更新本脚本并重新生成
 * 输出: docs/db-schema.svg （不带版本号，始终代表最新；文档内标注当前版本与日期）
 */
const fs = require('fs');
const path = require('path');

const SCHEMA_VERSION = 'v1.7.7'; // 当前覆盖版本（随 schema 变更更新）
const SCHEMA_DATE = '2026-09-09';
...
svg.push('<text x="60" y="56" font-size="30" font-weight="bold" fill="#0f172a">Victor&#8217;s CRM 数据库 Schema 关系图</text>');
svg.push(`<text x="60" y="92" font-size="16" fill="#475569">PostgreSQL 共享集群 (CloudBase: crm-d1gkae8ddc930d151) &#183; 20 张业务表 &#183; 9 个视图 &#183; 当前覆盖版本 ${SCHEMA_VERSION}（${SCHEMA_DATE}）</text>`);
...
svg.push(`<text x="60" y="${H - 30}" font-size="13" fill="#94a3b8">Victor&#8217;s CRM &#183; docs/db-schema.svg &#183; 由 tools/gen-schema-svg.js 生成（schema 变更后更新此脚本重新生成）</text>`);
svg.push('</svg>');

const out = path.join(__dirname, '..', 'docs', 'db-schema.svg');
