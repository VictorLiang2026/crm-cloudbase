...
$onlineUrl = 'https://crm-d1gkae8ddc930d151-1434199662.tcloudbaseapp.com/admin.html?v=' + [DateTimeOffset]::Now.ToUnixTimeSeconds()
...
try {
  ...
  if ($onlineHash -eq $localHash) {
    ...
    Sub "可能原因：本地改动尚未部署到托管，或浏览器/CDN 缓存（脚本已加时间戳防缓存）"
  }
} catch {
