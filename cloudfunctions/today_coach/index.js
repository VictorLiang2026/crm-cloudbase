...
async function loadAll() {
  const [cust, fol, rc, rf, rm] = await Promise.all([
...
// ---------- 规则版最终列表（AI 降级兜底） ----------
function ruleItems(custPool, rcPool) {
  ...
  return merged.map(x => ({
    type: x.type, id: x.id, name: x.name, stage: x.stage,
    priority: prio(x.score),
    reason: x.hits.join('；') || '有跟进价值',
    recommended_action: x.type === 'customer' ? actC(x) : actR(x),
    suggested_topic: x.last_note ? cut(x.last_note, 16) : (x.type === 'customer' ? x.stage : '近况交流'),
    next_step: x.type === 'customer' ? '更新下次跟进日期' : '推进下一阶段',
  }));
}

// ---------- AI 排序 ----------
function buildMessages(pools) {
  const lines = [];
  pools.forEach((x, i) => {
  ...
  const system = [
    ...
    '任务：综合紧迫度、经营价值、阶段节奏，选出今天最值得经营的最多 ' + FINAL_COUNT + ' 人，按建议联系先后排序。',
    '注意：增员候选人也是客户（先有关系后有增员），两类可交错排序；信号强的排前面。',
    '只输出 JSON，不要解释。输出格式：',
    '{"items":[{"type":"customer|recruit","id":数字,"priority":"高|中|低","reason":"为什么今天联系，1-2句话，不超过60字","recommended_action":"建议动作，1句话，不超过40字","suggested_topic":"今天聊的话题，不超过12字","next_step":"下一步，不超过20字"}]}',
    'reason 和 recommended_action 必须简洁、具体、可执行，不要空话。',
  ].join('\n');
...
function mergeAiResult(parsed, pools) {
  ...
  for (const it of items) {
    ...
    out.push({
      ...
      reason: cut(it.reason, 80),
      recommended_action: cut(it.recommended_action, 50),
      suggested_topic: cut(it.suggested_topic, 16),
      next_step: cut(it.next_step, 24),
    });
  }
...
exports.main = async (event, context) => {
  try {
    ...
    if (pools.length) {
      try {
        const { text } = await generateText(buildMessages(pools), { timeout: 100000 });
        const parsed = extractJson(text);
        items = mergeAiResult(parsed, pools);
        if (items) source = 'ai';
        else ai_error = 'AI 输出解析失败：' + cut(text, 120);
      } catch (e) { ai_error = 'AI 调用失败：' + e.message; }
    }
    if (!items) items = ruleItems(custPool, rcPool);
